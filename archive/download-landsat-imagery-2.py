...
def download_image(image, bands, aoi, folder, prefix):
    for band in bands:
        ...
        try:
            ...
            print(f"Downloading {prefix}_{band} from {url}")
            
            # Perform the download
            response = requests.get(url, stream=True)
            os.makedirs(folder, exist_ok=True)
            file_path = os.path.join(folder, f"{prefix}_{band}.tif")
            
            with open(file_path, "wb") as file:
                for chunk in response.iter_content(chunk_size=8192):
                    file.write(chunk)
            
            print(f"Saved {prefix}_{band} to {file_path}")
        
        except Exception as e:
            print(f"Error downloading {band} for {prefix}: {e}")
